...
async def generate_pdf(passenger: dict, flight: dict, template_path: str):
    ...
    original_out_src = out_src  # save before potential swap
    ...
    # FlyTicket one-way: remove the leg we don't need + change header
    if is_flyticket and is_oneway:
        if original_out_src == "IKA":
            # IKA to TBS: keep LEG 1, remove LEG 2
            html = re.sub(
                r'<hr class="leg-divider">.*?</div><!-- card-body -->',
                '</div><!-- card-body -->',
                html, flags=re.DOTALL
            )
        else:
            # TBS to IKA: keep LEG 2, remove LEG 1
            html = re.sub(
                r'(<div class="card-body">).*?<hr class="leg-divider">',
                r'\1',
                html, flags=re.DOTALL
            )
    ...
    if is_flyticket:
        # FlyTicket template uses short names like "VARESH" and "6808"/"6809"
        ...
        # Replace static values in FlyTicket template
    ...
    if is_flyticket:
        # FlyTicket template has static prices — replace first and second occurrence
    ...
    # FlyTicket: fix layout to fit A4 — shrink logo, ensure height fills page
    if is_flyticket:
        # Shrink logo height so page fits A4
        html = html.replace('height:33mm;', 'height:20mm;')
        # Make ticket exactly A4 height (not min-height)
        html = html.replace('min-height:297mm;', 'height:297mm;')
