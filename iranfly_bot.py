...
# ============================================================
# GOOGLE SHEETS CONFIG  ← تغییر اصلی اینجاست
# ============================================================
...
def parse_line(line):
    """Parse flight info in strict order: src/dst/airline/date/[sell]/[buy]/[voucher]/[insurance]
    Examples:
    - ika/tbs/qeshm/8jun
    - ika/tbs/qeshm/8jun/sell14.500/buy12.000
    - tbs/ika/varesh/11jun
    """
    parts = [p.strip().lower() for p in line.split('/')]
    
    if len(parts) < 4:
        raise ValueError(f"Invalid format. Use: src/dst/airline/date [/sell price] [/buy price]")
    ...
    # Parse remaining parts (sell, buy, voucher, insurance)
    for p in parts[4:]:
        p = p.strip()
        if p.startswith("sell"):
            ...
            # Plain number - treat as sell price if no sell yet
            if info["sell"] == 0:
                info["sell"] = to_num(p)
...
def _rebuild_flight_from_row(row: list) -> dict:
    ...
    route_out = safe(COL_ROUTE_OUT - 1)
    route_ret = safe(COL_ROUTE_RET - 1)
    air_out   = safe(COL_AIR_OUT - 1)
...
async def receive_passport(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            "✏️ فرمت (ترتیب الزامی):\n`src/dst/airline/date [/sell price] [/buy price]`\n\n"
            "📍 مثال‌ها:\n"
            "`ika/tbs/qeshm/8jun`\n"
            "`ika/tbs/qeshm/8jun/sell14.500`\n"
            "`ika/tbs/qeshm/8jun/sell14.500/buy12.000`\n\n"
            "🔄 دو طرفه (هر خط یک پرواز):\n"
            "`ika/tbs/qeshm/8jun/sell14.500/buy12.000`\n"
            "`tbs/ika/qeshm/11jun/sell14.000/buy13.000`",
            parse_mode="Markdown"
        )
