...
async def generate_pdf(passenger: dict, flight: dict, template_path: str):
    ...
    # Outbound ref and date
    html = html.replace('QA7X29', ref_out)
    if date_out:
        html = re.sub(r'26 May 2026(</span><span class="airport-time">11:00)?', 
                      f'{date_out}\\1', html, count=1)

    # Return ref and date  
    html = html.replace('VR3M84', ref_ret)
    if date_ret:
        html = re.sub(r'29 May 2026(</span><span class="airport-time">14:15)?',
                      f'{date_ret}\\1', html, count=1)

    # Update airline names
    html = html.replace('QESHM AIRLINE', outbound['airline'].upper().replace(' AIRLINE', ' AIRLINE'))
    html = html.replace('Q.2273', outbound['flight'])

    if not is_oneway:
        html = html.replace('VARESH AIRLINE', return_fl['airline'].upper().replace(' AIRLINE', ' AIRLINE'))
    ...
    prices = re.findall(r'<span class="price-value">.*?</span>', html, re.DOTALL)
    if len(prices) >= 1:
        html = html.replace(prices[0], f'<span class="price-value" style="font-size:18px;">{price_out_str}<br><span style="font-size:10px;font-weight:500;">تومان</span></span>', 1)
    if len(prices) >= 2 and not is_oneway:
        html = html.replace(prices[1], f'<span class="price-value" style="font-size:18px;">{price_ret_str}<br><span style="font-size:10px;font-weight:500;">تومان</span></span>', 1)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("✈️ صدور بلیت جدید", callback_data="new_ticket")],
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "new_ticket":
        await query.message.reply_text("📸 لطفاً عکس پاسپورت مسافر را ارسال کنید.")
...
async def receive_passport(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            "فرمت:\n`ika/tbs/qeshm/6jun/sell14000/buy12000`\n"
